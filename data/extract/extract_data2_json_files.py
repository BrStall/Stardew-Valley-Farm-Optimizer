...
def create_dataframe(content_data):
    """Cria um DataFrame a partir dos dados de conteúdo."""
    rows = []
    
    # Verifique se content_data é um dicionário ou uma lista
    if isinstance(content_data, dict):
        for key, value in content_data.items():
            flattened = flatten_json(value)  # Flattens the nested structure
            rows.append(flattened)
    elif isinstance(content_data, list):
        for item in content_data:
            flattened = flatten_json(item)  # Flattens the nested structure
            rows.append(flattened)
    else:
...
def process_json_files(engine):
    ...
    json_folder_path = os.path.join(os.getcwd(), 'json_files/data2')
